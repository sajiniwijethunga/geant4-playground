   ...
   fDetectorMessenger(nullptr)
{
  ...
  fAbsorberThickness = 0.5*cm;
  fAbsorberSizeYZ    = 5.0*cm;
  fXposAbs           = 0.*cm;
...
void DetectorConstruction::ComputeGeomParameters()
{
  ...
  fWorldSizeX = 10. *cm; //2.4*xmax; 
  fWorldSizeYZ= 10.* cm; //1.2*fAbsorberSizeYZ;
...
{ 
  ...
  // fLogicCylinder1 = new G4LogicalVolume(fSolidCylinder1,    //its solid
  //                                      Air, //its material
  //                                      "Cylinder1");       //its name
  

  // fPhysiCylinder1 = new G4PVPlacement(0,                   //no rotation
  //                       G4ThreeVector(0.*cm, 0.*cm,-0.5*cm),    //its position
  //                               fLogicCylinder1,     //its logical volume
  //                               "Cylinder1",         //its name
  //                               fLogicCylinder,        //its mother
  //                               false,              //no boulean operat
  //                               0);                 //copy number
